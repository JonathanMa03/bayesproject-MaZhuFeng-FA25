...
```{r Cancer choice}
# TODO: Choose the cancer site (using breast as example)
cancer_site <- df %>% filter(site == 50)
str(cancer_site)
```
...
We check R and ESS in the model summary.
...
# ranef(brm_model2)
#TODO add model run file so TAs dont have to full run
# Save to Posterior file and check VIF, ESS, Rhat, ACF plot
```
